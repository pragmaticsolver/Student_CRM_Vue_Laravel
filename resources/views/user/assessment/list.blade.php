                ...
                @if($assessment_users->count() > 0)
                    ...
                    @foreach($assessment_users as $assessment_user)
                        <tr>
                            ...
                            <td>{{ \Illuminate\Support\Str::limit($assessment_user->assessment->description, $limit = 50, $end = '...') }}</td>
                            <td>{{ $assessment_user->schedule ? $assessment_user->schedule->class->title : '' }}</td>
                            <td>{{ $assessment_user->schedule ? $assessment_user->schedule->get_date() : ''}}</td>
